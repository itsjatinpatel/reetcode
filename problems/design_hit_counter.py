...
hitCounter.getHits(4);   // get hits at timestamp 4, return 3.2
...
class HitCounter:
    ...
    def hit(self, timestamp: int) -> None:
        if not self.buffer or self.buffer[-1][0] != timestamp:
            self.buffer.append([timestamp, 0])
    ...
    def getHits(self, timestamp: int) -> int:
        while (
            self.buffer and timestamp - self.buffer[0][0] >= HitCounter.LOOKBACK_WINDOW
        ):
            self.count -= self.buffer.popleft()[1]
